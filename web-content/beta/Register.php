...
function show_register_form()
{
	...
	echo "				<label for=\"rgUsername\" class=\"rgUsername\">* Enter a Username: </label><input type=\"text\" id=\"rgUsername\" placeholder=\"JPublicXYZ\" name=\"rgUsername\" size=\"30\" onchange=\"check_username(this.form.rgUsername.value)\"/><span id=\"username_label\" class=\"username_label\"></span><br />";
	...
	echo "				<option value=\"3\" >$16.95/mo - 3 out (Standard Monthly)</option>";
	echo "				<option value=\"3\" >$24.95/mo - 3 out (Premium Monthly)</option>";
...
//	echo "			<label for=\"step2Address2\" class=\"step2Address2\">Address 2 (optional): </label><input id=\"rgMailingAddress2\" type=\"text\" name=\"rgMailingAddress2\" size=\"39\" /><br />";
	...
	echo "			<input id=\"step2CheckBox\" class=\"step2CheckBox\" type=\"checkbox\" name=\"checkboxName\" value=\"checkboxValue\" /><label for=\"step2BillingMailingAddress\" class=\"step2BillingMailingAddress\">My Billing Address is the same as my Mailing Address</label><br />";
	...
	echo "				<option value=\"Disc\">Discover</option>";
	...
	echo "			<label for=\"step2CCExpiration\" class=\"step2CCExpiration\">* Expiration Date: ";
	echo "			<select class=\"rgExpireMonth\" id=\"rgExpireMonth\" name=\"rgExpireMonth\" size=\"1\">";
	echo "				<option value=\"\" selected=\"selected\">Month</option>";
	echo "				<option value=\"0\">January</option>";
	echo "				<option value=\"1\">February</option>";
	echo "				<option value=\"2\">March</option>";
	...
	echo "				<option value=\"11\">December</option>";
	echo "			</select>";
	echo "			<select id=\"rgExpireDay\" name=\"rgExpireDay\" size=\"1\">";
	echo "				<option value=\"\" selected=\"selected\">Day</option>";
	echo "			</select>";
	echo "			<select id=\"rgExpireYear\" class=\"rgExpireYear\" name=\"rgExpireYear\" size=\"1\">";
	echo "				<option value=\"\" selected=\"selected\">Year</option>";
...
//    $ccExpiration = $_POST['rgCreditCardExpirationDate'];
    $ccExpirationMonth = $_POST['rgExpireMonth'];
    $ccExpirationDay = $_POST['rgExpireDay'];
    $ccExpirationYear = $_POST['rgExpireYear'];
    $ccExpiration = $ccExpirationYear." " .$ccExpirationMonth;
    $success = false;
       
	// Use PEAR DB to insert data into customer table
	$data = array('customerName' => $user, 'mailingAddress1' => $mailingAddress1, 'mailingAddress2' => $mailingAddress1, 'mailingCity' => $mailingCity,
	'mailingState' => $mailingState, 'mailingZipcode' => $mailingZipcode, 'phoneNumber' => $phoneNumber, 'billingAddress1' => $billingAddress1,
	'billingAddress2' => $billingAddress1, 'billingCity' => $billingCity, 'billingState' => $billingState, 'billingZipcode' => $billingZipcode,
...
//			}
		...
		function formValidatorStep2()
		{
			...
			var expireMonth = document.getElementById('rgExpireMonth');
			var expireDay = document.getElementById('rgExpireDay');
			var expireYear = document.getElementById('rgExpireYear');
			// Check each input in the order that it appears in the form!
...
//									if (checkBirthDate(expireMonth, expireDay, expireYear)){	
...
//																					if(checkExpirationDate(creditCardExpirationMonth, creditCardExpirationYear)){
																						return true;
//																					}
																				}
																			}
...
//		function isBillAddressChecked(elem, helperMsg)
//		{
//			var user_input = "";
//			user_input = elem.checked;
//			if(user_input)
...
//		}
		...
		function checkBirthDate(expireMonth, expireDay, expireYear)
		{
			var thisDate = new Date();
			
			// Born at least 19 years ago
			if ((getYear(thisDate) - expireYear) < 19)
			{
				// If at least 18 years old & Birth month already passed
				if (((getYear(thisDate) - expireYear) == 18) && ((getMonth(expireMonth) < expireMonth)))
				{
					// If 18 yr old & current birth month & today is birthday
					if (((getYear(thisDate) - expireYear) == 18) && ((getMonth(expireMonth) == expireMonth) && (getDate(thisDate) <= expireDay)))
					{
						return true;
					} else {
		...
		function duplicateUsername(elem)
		{
			if (document.getElementById('username_label').innerHTML == 'This username is unavailable!')
			{
//				elem.focus();
				username.focus();
				return false;
		...
		function isValidCreditCard(ccType, elem)
		{
			if (ccType.value == "Visa")
	   		{
    			// Visa: length 16, prefix 4, dashes optional.
	   			var re = /^4\d{3}-?\d{4}-?\d{4}-?\d{4}$/;
	   		} else if (ccType.value == "MC") {
    	  		// Mastercard: length 16, prefix 51-55, dashes optional.
	    	  	var re = /^5[1-5]\d{2}-?\d{4}-?\d{4}-?\d{4}$/;
	   		} else if (ccType.value == "Disc") {
   				// Discover: length 16, prefix 6011, dashes optional.
	    	  	var re = /^6011-?\d{4}-?\d{4}-?\d{4}$/;
	   		} else if (ccType.value == "AmEx") {
    	  		// American Express: length 15, prefix 34 or 37.
	    	  	var re = /^3[4,7]\d{13}$/;
	   		} else if (ccType.value == "Diners") {
   			...
   			{	
   				alert("Please re-enter Credit Card Number.\n");
				elem.focus();
				return false;
	   		}
   			...
   			var checksum = 0;
	   		for (var i=(2-(elem.length % 2)); i<=elem.length; i+=2)
	   		{
    			checksum += parseInt(elem.charAt(i-1));
	   		}
	   		// Analyze odd digits in even length strings or even digits in odd length strings.
		   	for (var i=(elem.length % 2) + 1; i<elem.length; i+=2)
   			{
   				var digit = parseInt(elem.charAt(i-1)) * 2;
		      	if (digit < 10) 
    		  	{
      				checksum += digit;
		      	} else {
   			...
   			if ((checksum % 10) == 0)
	   		{
    			...
    			alert("Credit Card Number not valid. Not correct length.\n");
				elem.focus();
	  			return false;
	   		}
		}
		...
		<script src="../includes/resolveCCDate.js" type="text/javascript" language="javascript"></script>		
