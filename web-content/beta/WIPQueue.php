<?php # Queue.php
	...
	class QueueList
	{
		...
		// class constructor initializes the data tier object
  		...
  		function movieDelete($movieId, $dateRequested)
  		{
	    	$this->result = $this->mDoQueue->DeleteFromQueue($movieId, $dateRequested);	
  		}
    }
	...
	if ($a == 1)
	{
		// If user does not have movies in the queue
		$queue_query =& $db->query("SELECT mq.queueId FROM member m INNER JOIN myqueue mq on m.myqueueId = mq.myqueueId WHERE m.memberName = '".$auth->getUsername()."'  and q.dateSent IS NULL", array());
		$a = "";
		$queue_row = array();
	} else {
		// If user already has movies in the queue
		$queue_query =& $db->query("SELECT q.queueId, q.queueOrder, q.dateRequested, q.dateSent, q.dateReturned, q.movieId, q.inventoryNum, q.availabilityStatus FROM member m INNER JOIN myqueue mq on m.myqueueId = mq.myqueueId INNER JOIN queue q on mq.queueId = q.queueId WHERE m.memberName = '".$auth->getUsername()."'  and q.dateSent IS NULL", array());
		$queue_row = array();
	}		
	
	while ($queue_query->fetchInto($queue_row))
	{
		$queueId = $queue_row['queueId'];				// get the queue Id
	}
	...
	$queue_query->free();								// free the query from memory
...
//	echo $queueId;
	...
	if ($rmQueue==1)
	{
	    $delete_queue->movieDelete($movieId, $dateRequested);
		// Reset the movieId to zero, otherwise the logic below will 
		// add the deleted movie back to the user queue.
  		$rmQueue = 0;
...
//	if ($movieId == 0)
	...
	if ($movieId > 0)
	{
		...
		$queue_query =& $db->query("SELECT * FROM queue WHERE queueId = $queueId AND movieId = $movieId AND dateSent IS NULL", array());
...
//				$result = $this->dbManager->GetAll($no_InventoryNum_query);

//				$no_InventoryNum_query = $result[0];
...
//			$addToQueue = array('queueId' => $queueId, 'queueOrder' => $queueOrder, 'dateRequested' => date('Y-m-d'), 'movieId' => $movieId, 'inventoryNum' => $inventoryNum);
//			$addToQueue = array('queueId' => $queueId, 'queueOrder' => $queueOrder, 'dateRequested' => date('Y-m-d'), 'movieId' => $movieId);
			$addToQueue = array('queueId' => $queueId, 'queueOrder' => $queueOrder, 'dateRequested' => date('dmYhiS'), 'movieId' => $movieId);
	...
	<body leftmargin="0" marginheight="0" marginwidth="0" topmargin="0">
		...
		<form id="FormName" action="(EmptyReference!)" method="get" name="FormName">
			<div style="position:relative;width:950px;height:885px;-adbe-g:p,10,10;">
				...
				<div style="position:absolute;top:40px;left:30px;width:32px;height:30px;">
					<table width="731" border="1" cellspacing="1" cellpadding="0" bgcolor="#f2f2f2">
						...
						<?php
						// Print entire Movie queue   = 1){Available Now}else{Short Wait}  ($queue_list->result[$i]['availability'] == 1){ echo "Available Now"; } else { echo "Short Wait"; } 
						for ( $i = 0; $i < $queueOrder -1; $i++ )
						{
							$dateRequested = $queue_list->result[$i]['dateRequested'];
							//echo $dateRequested;
							...
							print "<td align=\"center\" width=\"125\"><label>Available Now</label></td>";
							print "<td width=\"62\"></td>";
							print "<td width=\"72\"><a href=\"Queue.php?movieId=".$queue_list->result[$i]['movieId']."&queueId=".$queueId."&dateRequested=".$queue_list->result[$i]['dateRequested']."&rmQueue=1 \">X</a></td>";
							print "</tr>";
						}
					?>
					</table>
